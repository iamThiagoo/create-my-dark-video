<template>
  <div class="w-full">
    <div class="container flex flex-col items-center justify-center w-full mx-auto">      
      <div class="min-h-[calc(100vh-1.25rem)] flex flex-col items-center justify-center h-full gap-8 py-20 lg:py-10 px-3 lg:px-0">
        <div class="relative flex flex-col gap-4">
          <div class="flex justify-center">
            <Badge class="text-center w-fit">{{ $t('header.inDevelopmentBadge') }}</Badge>
          </div>
          <svg class="h-28 dark:fill-slate-300" viewBox="0 0 14 14" role="img" focusable="false"
          ...
          <h2
            class="relative max-w-4xl text-4xl font-bold tracking-tighter text-center text-slate-700 md:text-6xl font-regular dark:text-slate-300">
            <span class="inline-flex flex-row items-center justify-center">{{ $t('header.generateYourDarkVideo') }}
              <span v-for="(title, index) in titles" :key="index" :style="getTitleStyle(index)"
                class="absolute block mt-2 font-bold sm:hidden text-slate-700 dark:text-slate-300">
                {{ title }}
              </span>
            </span>
            <div class="relative hidden sm:mt-4 sm:-top-32 sm:block">
              <span v-for="(title, index) in titles" :key="index" :style="getTitleStyle(index)"
                class="absolute flex justify-center w-full overflow-hidden font-bold text-center text-slate-700 md:pb-4 md:pt-1 dark:text-slate-300">
                {{ title }}
              </span>
            </div>
          </h2>
          ...
          <p
            class="relative max-w-3xl mt-10 text-sm leading-relaxed tracking-tight text-center sm:mt-16 text-slate-600 dark:text-slate-500 md:text-xl text-muted-foreground">
        ...
        <div class="flex flex-row gap-3">
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger>
                <Button size="lg" @click="$emit('try-now')" class="gap-4 px-16 py-6 -mt-2 font-bold text-white rounded-full shadow-xl sm:mt-0 sm:px-24 sm:text-lg bg-slate-800 hover:text-white hover:bg-slate-600 dark:text-white"
                  variant="outline">
                  {{ $t('header.tryNow') }}
                </Button>
              </TooltipTrigger>
