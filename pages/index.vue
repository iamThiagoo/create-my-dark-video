                        ...
                        <form class="w-full" ref="form" @submit.prevent="onSubmit">
                            <div class="grid items-center w-full gap-4">
                                <div class="flex flex-col gap-y-4">
                                    ...
                                    <p v-if="errorMessage" class="ml-1 -mt-2 text-sm text-red-600">{{ errorMessage }}</p>
                                </div>
...
const onSubmit = async () => {
    try {
        if (!fields.prompt) {
            errorMessage.value = 'Por favor, insira um prompt ou roteiro para continuar.'
            return;
        } else {
            errorMessage.value = null
        }

        const video = await $http.video.generate(fields)

    } catch (error) {
        console.error(error)
        toast({
            title: 'Uh oh! Aconteceu um erro inesperado...',
            description: 'Tente novamente, se não funcionar, entre em contato conosco :)',
            variant: 'destructive',
        });
    }
}
